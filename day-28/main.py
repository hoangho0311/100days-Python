...
def reset_time():
    ...
    canvas.itemconfig(timer_item, text="00:00")
...
label = Label(text="Timer", font=(FONT_NAME, 35, "bold"), bg=GREEN, fg =YELLOW)
label.grid(column=2, row=1)

canvas = Canvas(width=200, height=224, bg=GREEN, highlightthickness=0)
tomatoImg = PhotoImage(file="tomato.png")
canvas.create_image(100, 112,image=tomatoImg)
timer_item = canvas.create_text(103, 130, text="00:00", fill="white", font=(FONT_NAME, 35, "bold"))
canvas.grid(column=2, row=2)

startBtn = Button(text="Start", command=start_timer)
startBtn.grid(column=1, row=3)
resetBtn = Button(text="Reset", command=reset_time)
